...
import { draftlist, DeleteStory } from '@/actions/story';
...
const AllBlogs = () => {
    ...
    const handledelete = async (slug) => {
        try {
            const data = await DeleteStory(slug, token);
            if (data && data.error) {
    ...
    return (
        <AdminDashboard>
            ...
            <div className="text-center text-sm mb-2">Total Web Stories &nbsp;-&nbsp; {blogscount}</div>
            ...
            {blogscount === 0 && <div className="text-center text-[black] mt-10 text-lg font-bold">No Web Stories Found</div>}
                        ...
                        {blogs && blogs.map((blog, index) => (
                            ...
                            <tr key={index} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                ...
                                <td className="flex items-center gap-10 px-6 py-4">
                                    <div className="cursor-pointer text-black"> <a target="_blank" href={`/admin/web-stories/${blog.slug}`}> <FaEdit size={20} /></a></div>
            ...
            <div className={`fixed inset-0 flex items-center justify-center z-50 md:pl-[150px]  ${isOpen ? '' : 'hidden'}`}>
                ...
                <div className="bg-white p-8 rounded-lg shadow-lg z-10">
                    ...
                    <div className="text-sm text-[#d35e5e]">Type the name of the Web Story</div>
                    ...
                    <div className="flex justify-end mt-8">
                        <button disabled={inputValue !== currentuserID.title} onClick={() => handleConfirmDelete(currentuserID.slug)} className={`text-sm bg-[#ca0a0a] mr-2 hover:bg-[#0A4158] hover:scale-105 active:scale-95 transition-transform text-white font-semibold py-2 px-4 rounded ${inputValue !== currentuserID.title ? 'opacity-50 cursor-not-allowed' : ''}`}>
                            Delete
                        </button>
